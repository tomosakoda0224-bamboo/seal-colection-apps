...
DEFAULT_WORKSHEET_NAME = "stickers"
...
def get_google_sheets_settings() -> dict[str, object] | None:
    try:
        credentials = dict(st.secrets["gcp_service_account"])
        google_sheets = st.secrets["google_sheets"]
        spreadsheet_id = str(google_sheets["spreadsheet_id"])
        worksheet_name = str(google_sheets.get("worksheet_name", DEFAULT_WORKSHEET_NAME))
    except Exception:
        return None
...
def ensure_worksheet_header(worksheet) -> None:
...
def save_data(df: pd.DataFrame) -> None:
    ...
    if worksheet is not None:
        worksheet.clear()
        worksheet.update([COLUMNS] + df.astype(str).values.tolist(), "A1")
    else:
        ensure_data_file()
